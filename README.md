...
- **A6 Autonomous intracellular AND-gate (MHC-free, Shriya's original)** — synthetic gene circuit: intracellular signal A AND B → self-destruct, no MHC, for the dark core. `Test:` all pairs of 8 transcriptional programs on real single-cell data (309k vital + 153k tumour cells), worst-donor vital leak. **Status: ⚠️ RUNG-23 v1 negative UNDER RE-VERIFICATION — audit found 2 method flaws (no depth-gate → dropout-deflated coverage; single-worst-donor leak → a 62% PROLIF 'leak' into POST-MITOTIC cardiomyocytes/neurons that CANNOT proliferate = artifact, the exact bug RUNG-8 fixed). v2 HARDENED (`scripts/49`, selftest 11/11): housekeeping depth-gate + robust p90-donor-per-vital-type leak. RE-RUN PENDING on Colab before any conclusion. (v1 said 0 gates; do NOT trust until v2.)**
