...
- **A5 Boltz structural binding cross-check (2b)** — an INDEPENDENT SOTA co-folder (Boltz-2) re-checks: does the MUTANT peptide present on its MHC while the WT doesn't? 3-way agreement vs MHCflurry (RUNG-11) + ESMFold (RUNG-12). `Test:` Boltz-2 pMHC interface pLDDT/ipTM, mut-vs-WT. **Status: 🟢 RUNG-20 BUILT + selftest 15/15 (`scripts/46`, `notebooks/boltz_specificity_colab.ipynb`, GPU) — folds IDH1-R132H×2 / BRAF-V600E / KRAS-G12D×2; run pending on Colab.**
