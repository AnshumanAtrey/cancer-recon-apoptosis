...
- **A7 Autonomous mutation-sensing circuit (MHC-free, the corrected route)** — sense the MUTATION directly (toehold/CRISPR), AND-gate two clonal mutations → self-destruct. `Test:` ViennaRNA allele-specific ΔΔG. **Status: ✅ RUNG-25 — FEASIBLE WITH A CONSTRAINT. Single-base RNA sensing discriminates ~10/12 substitution types (ΔΔG 4–6.5 kcal/mol, false-fire 1e-5..1e-3); AND of two NON-wobble clonal mutations → ~1e-6 = tumour-exclusive. BUT FAILS for the 2 G·U-WOBBLE types (G>A, U>C) where the sensor·WT mismatch is a stable wobble — and KRAS-G12D is G>A → poorly sensed at the RNA level (false-fire 0.38). DESIGN RULE: at RNA level pick non-wobble substitutions; for wobble drivers (KRAS-G12D) sense at the DNA level (CRISPR, no wobble). (rule-5 audit caught an over-optimistic auto-headline — the per-substitution table is the trustworthy core; the 2 worst types are EXACTLY the G·U wobbles, textbook-correct.)**
- **A6 Autonomous intracellular AND-gate (MHC-free, Shriya's original)** — synthetic gene circuit: intracellular signal A AND B → self-destruct, no MHC, for the dark core. `Test:` all pairs of 8 transcriptional programs on real single-cell data (309k vital + 153k tumour cells), worst-donor vital leak. **Status: ✅ RUNG-23 v2 CONFIRMED NEGATIVE (trustworthy — hardened + re-run, depth-gated + robust p90-donor leak; 954k cells, 70% well-seq). 0 clean gates. The v1 artifact is FIXED (v1's 62% PROLIF 'leak' was post-mitotic cardiomyocytes = impossible; v2 correctly attributes leak to vascular_endothelium). The REAL reason the gate fails: depth-gating LIFTS coverage (GLYCOLYSIS 69%, MYC 47% of tumour) but those high-coverage metabolic programs GENUINELY leak into metabolically-active vital organs (kidney tubule 97%, endothelium) — while the clean programs (E2F/WNT/TERT, leak 1–5%) cover only ~1–5% of tumour. Robust specificity-coverage anti-correlation. CONFIRMS the unifying thesis (RUNG-5/15/32/23): every EXPRESSION window leaks; the MUTATION is the only tumour-exclusive signal → autonomous circuit must sense the MUTATION directly, not transcriptional state. (rule-5 audit caught the v1 artifact; the negative survived for the correct reason.)**
- **A5 Boltz structural binding cross-check (2b)** — an INDEPENDENT SOTA co-folder (Boltz-2) re-checks: does the MUTANT peptide present on its MHC while the WT doesn't? 3-way agreement vs MHCflurry (RUNG-11) + ESMFold (RUNG-12). `Test:` Boltz-2 pMHC interface pLDDT/ipTM, mut-vs-WT. **Status: ✅ RUNG-20 RAN (Colab T4, commit pending) — Boltz INDEPENDENTLY CONFIRMS all 5 mutant neoantigens present on MHC (interface pLDDT 0.95–0.99, well above the 0.70 bar) → the *presentation* step is cross-validated by a 3rd SOTA model. BUT Boltz's confidence SATURATES (it docks WT just as confidently, iplddt≈mut) → it cannot discriminate mut-vs-self by binding (known structure-folder limitation = our step-1 ipTM caveat). The discrimination these handles use is TCR-level (exposed mutated residue, ESMFold RSA in RUNG-12), which interface-pLDDT doesn't measure → 0 certified / 1 supported (BRAF, MHCflurry agreed) / 4 flagged. Honest net: presentation ✓ cross-validated; a proper Boltz discrimination read needs pose-RSA, not confidence (noted refinement).**
...
- **B6 Wave-as-injection delivery threshold** — how sparse can a ONE-TIME injection be? `Test:` lattice, seed-fraction × coupling. **Status: ✅ RUNG-24 — at super-critical coupling, f* < 0.05% (≈ ONE seed) clears 70–97% of tumours; injecting MORE barely changes it. Delivery is NOT the bottleneck — TRIGGER POTENCY (coupling) is the real gate. A sparse one-time shot suffices if the trigger is potent (Anshuman's decouple-kill-from-delivery confirmed); below critical coupling no delivery helps.**
